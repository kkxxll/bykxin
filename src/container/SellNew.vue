<template>
  <div class="sellnew">
        ...
        <el-form-item label="商品图片" class="sellnew-imgs">
          <ul>
            <li v-for="(item, index) in form.photos" :key="index">
              <img :src="item" alt="">
              <div class="sellnew-imgs__fixed">
                <div class="sellnew-imgs__main" @click="setMainImg(index)">设为主图</div>
                <div class="sellnew-imgs__del" @click="delImg(index)">删除</div>
              </div>
              <i class="el-icon-success sellnew-imgs__cur" v-show="index === mainimg"></i>
            </li>
          </ul>
    ...
    <div class="sellnew-btn">
      <el-button type="warning" @click="createSell">发布</el-button>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    createSell () {
      ...
      axios.post('/api/sell/new', params).then((response) => {
        let res = response.data
        switch (res.status) {
          case '0':
            this.$message({
